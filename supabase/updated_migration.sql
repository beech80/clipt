-- Combined migration script for all Clipt features
-- Includes base tables, stream functionality, and all feature tables

-- PART 1: BASE TABLES AND STREAMS
-- Include contents from combined_migration.sql first
...
-- PART 2: FEATURE TABLES - STREAM CLIPS, DONATIONS, POLLS, SCHEDULED STREAMS
...
CREATE POLICY IF NOT EXISTS "Anyone can view clips" 
ON public.stream_clips FOR SELECT 
USING (true);

CREATE POLICY IF NOT EXISTS "Users can create their own clips" 
...
CREATE POLICY IF NOT EXISTS "Users can like/unlike clips" 
...
CREATE POLICY IF NOT EXISTS "Users can see their own donations and streamers can see donations to their streams" 
...
CREATE POLICY IF NOT EXISTS "Users can create their own donations" 
...
CREATE POLICY IF NOT EXISTS "Anyone can view public scheduled streams" 
ON public.scheduled_streams FOR SELECT 
USING (is_private = false OR auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can manage their own scheduled streams" 
