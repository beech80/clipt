...
serve(async (req) => {
  ...
  try {
    ...
    const gameResponse = data.choices[0].message.content;

    // Extract game context if mentioned in the message
    const gameContext = message.match(/\b(game|gaming|play|played)\b/i) ? message : null;
    ...
    console.error('Error in game-chatbot function:', error);
    return new Response(
