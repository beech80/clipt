...
const GameBoyControls = ({ currentPostId }: GameBoyControlsProps) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-32 bg-[#1A1F2C]/95 backdrop-blur-sm border-t border-[#403E43] z-50">
      {/* Bottom Center Navigation Menu */}
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50">
        <Sheet onOpenChange={setIsOpen}>
          <SheetTrigger asChild>
            <button className="rounded-full bg-[#403E43] p-3 hover:bg-[#504E53] transition-colors">
            ...
            <nav className="grid grid-cols-2 gap-2 p-3">
              {navigationItems.map((item) => (
                <button
                  ...
                  className="p-3 rounded-lg bg-[#403E43]/50 hover:bg-[#403E43] transition-colors text-white"
                >
                  {item.name}
                </button>
              ))}
      ...
      <div className="fixed left-8 bottom-4 w-32 h-32">
        <Joystick onDirectionChange={handleVideoControl} />
      </div>

      {/* Action Buttons */}
      <div className="fixed right-8 bottom-4 w-32 h-32">
