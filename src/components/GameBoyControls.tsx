...
const GameBoyControls: React.FC<GameBoyControlsProps> = ({ currentPostId }) => {
  ...
  return (
    <>
      {/* Top action buttons in diamond formation */}
      <div className="fixed right-4 top-5 z-50">
        <ActionButtons postId={currentPostId || ''} />
      </div>
      
      {/* Bottom GameBoy controller */}
      <div className="gameboy-container h-[180px] sm:h-[200px] bg-[#1e2130]/95 backdrop-blur-sm fixed bottom-0 left-0 right-0 z-50 touch-none border-t-2 border-[#3e4462]">
        <div className="fixed bottom-2 sm:bottom-4 left-1/2 -translate-x-1/2 z-50">
          <Sheet>
            <SheetTrigger asChild>
              <button className="rounded-full bg-[#4c4f74]/50 p-2.5 sm:p-3 backdrop-blur-sm border border-[#5f6384]/30 
                hover:bg-[#5f6384]/50 transition-all duration-300 touch-none active:scale-95">
                <Menu className="w-5 h-5 sm:w-6 sm:h-6 text-[#8c91c0]" />
              </button>
            </SheetTrigger>
              ...
              <nav className="grid grid-cols-2 gap-2 p-3">
                {navigationItems.map((item) => (
                  <button
                    ...
                    className="flex items-center gap-2 p-3 sm:p-4 rounded-lg bg-[#30323d]/20 hover:bg-[#30323d]/30 
                      active:bg-[#30323d]/40 transition-all duration-300 text-white/80 
        ...
        <div className="fixed left-1/2 -translate-x-1/2 bottom-24 sm:bottom-28">
          <button 
            ...
            className="relative flex items-center justify-center w-[80px] h-[60px] active:scale-95 transition-transform"
