...
const DiscoveryNew: React.FC = () => {
                    ...
                    <div className="streamer-profile" style={{
                      ...
                      <div className="streamer-details" style={{ flex: 1 }}>
                        ...
                        <p style={{ 
                          ...
                          <span style={{
                            background: 'linear-gradient(90deg, #202040 0%, rgba(255,85,0,0.8) 100%)',
                            padding: '5px 14px',
                            borderRadius: '8px',
  ...
  >
    ...
    <div className="controller-right">
      ...
      {/* Chat Panel - Using the new BottomChatPanel component */}
      <BottomChatPanel
        isOpen={showChat}
        onClose={() => setShowChat(false)}
        streamer={{
          id: chatPartnerId,
          username: streamers.find(s => s.id === chatPartnerId)?.username || '',
          displayName: streamers.find(s => s.id === chatPartnerId)?.display_name || '',
          profilePicture: streamers.find(s => s.id === chatPartnerId)?.avatar_url || '',
          viewerCount: streamers.find(s => s.id === chatPartnerId)?.viewer_count || 0
        }}
      />
    </div>
  );
