...
function Profile() {
  ...
  console.log('Profile component rendering with ID:', profileId, 'URL param:', id, 'User ID:', user?.id);
  ...
  const fetchProfileData = async () => {
      ...
      // Simplified error handling approach with fallbacks at each step
      try {
        // 1. Get profile data - extremely simplified with clear error handling
        let profileData;
        try {
          ...
          console.warn('Using fallback profile due to error:', err);
        ...
        // Set profile data regardless of source
        setProfile(profileData);
        
        // 2. Set up sample posts
        setUserPosts(createSamplePosts());
        
        // 3. Get follower counts - with fallbacks
        let followers = 0;
        let following = 0;
        ...
        try {
          ...
          console.warn('Error fetching followers, using default value:', err);
        }
        ...
        try {
          ...
          console.warn('Error fetching following, using default value:', err);
        }
        
        setFollowersCount(followers);
        setFollowingCount(following);
        
        // 4. Get saved clips if own profile
        if (isOwnProfile) {
          try {
            const savedClips = await getSavedClipts(profileId);
            setSavedCliptsData(savedClips || []);
          } catch (err) {
            console.warn('Error fetching saved clips, using empty array:', err);
            setSavedCliptsData([]);
          }
        }
      } catch (err) {
        console.error('Fatal error in profile data fetching:', err);
        setError('Failed to load profile. Please try again.');
        toast.error('Could not load profile data');
      } finally {
        console.log('Setting loading to false - profile data fetch complete');
        setLoading(false);
      }
    };
  ...
  // Safeguard function to ensure profile always renders something
  const safeRender = () => {
    // If neither profile nor error is present and we're not loading,
    // force a minimal fallback profile to be displayed
    if (!profile && !loading && !error) {
      console.log('Forcing fallback profile to display');
      ...
      return (
        <div style={{
          ...
          <div style={{ marginBottom: '20px' }}>
            <AlertTriangle size={40} color="#FF5500" />
          </div>
  ...
  // Check for render blockers first
  const blockingContent = safeRender();
  if (blockingContent) return blockingContent;
  
  // If we got this far, we're ready to render the main profile
  return (
    <>
      <GlobalStyle />
