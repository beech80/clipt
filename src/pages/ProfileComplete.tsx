...
Profile() {
  ...
  // Safe profile ID handling - very important to have a fallback
  ...
  // Create a fallback profile for development/errors
  const createFallbackProfile = (id: string): ProfileType => ({
    id: id,
    username: 'gamer_' + id.substring(0, 5),
    full_name: 'Demo Gamer',
    avatar_url: 'https://placehold.co/400x400/121212/FF5500?text=Gamer',
    bio: 'This is a demo gaming profile with trophies and achievements.',
    website: 'https://example.com',
    location: 'Gaming World',
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
    streaming_url: 'https://twitch.tv/example',
    banner_url: 'https://placehold.co/1200x400/121212/4169E1?text=Gaming+Banner'
  });
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      ...
      if (!profileId || profileId === 'undefined') {
        ...
        // Just use a fallback profile
        setProfile(createFallbackProfile('default-profile'));
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        // First attempt to load from localStorage for immediate display
        try {
          ...
          console.error('Error loading cached profile:', cacheErr);
        }
        ...
        // Handle profile data results
        if (profileError) {
          console.error('Supabase profile error:', profileError);
          // If we don't already have a profile from cache, use fallback
          if (!profile) {
            console.log('Using fallback profile');
            setProfile(createFallbackProfile(profileId));
          }
        } else if (profileData) {
          console.log('Successfully loaded profile from Supabase');
          setProfile(profileData);
          
          // Cache the profile data for faster loading next time
          try {
        ...
        // Always load sample posts for demo
        const samplePosts = createSamplePosts();
        setUserPosts(samplePosts);
        
        // Set some default counts if we can't fetch them
        setFollowersCount(25);
        setFollowingCount(42);
        
        // Try to fetch actual follower/following counts
        try {
          ...
          if (followersCount !== null) setFollowersCount(followersCount);
          if (followingCount !== null) setFollowingCount(followingCount);
        } catch (followErr) {
          console.error('Error fetching follow counts:', followErr);
          // Keep the default counts we already set
        }
        ...
        if (isOwnProfile) {
          try {
            ...
            if (savedCliptsResult && savedCliptsResult.length > 0) {
              setSavedCliptsData(savedCliptsResult);
            } else {
              // Use sample saved clipts for demo
              setSavedCliptsData(samplePosts.slice(0, 2));
            }
          } catch (cliptErr) {
            console.error('Error fetching saved clipts:', cliptErr);
            setSavedCliptsData(samplePosts.slice(0, 2)); // Fallback to samples
          }
        }
      } catch (err) {
        console.error('Unexpected error in profile fetch:', err);
        setError('Failed to load profile');
        // Ensure we have fallback data even on total failure
        if (!profile) {
          setProfile(createFallbackProfile(profileId));
        }
        // Show toast but don't block rendering with fallback data
    ...
    fetchProfileData();
  }, [profileId, isOwnProfile, profile]);
