...
function DiscoveryFixed() {
  // Move all state declarations and initialization inside the component body
  const navigate = useNavigate();
  const { user } = useAuth();
  
  // Use React.useState instead of importing it
  const [gameFilter, setGameFilter] = React.useState('top_rated');
  const [activeTab, setActiveTab] = React.useState('games');
  const [searchQuery, setSearchQuery] = React.useState('');
  const [trendingFilter, setTrendingFilter] = React.useState('most_liked');
          ...
          {searchQuery && searchQuery.length > 2 ? (
            ...
            <>
              <div className="flex justify-between items-center mb-4">
                ...
                <Select value={gameFilter} onValueChange={(value) => setGameFilter(value)}>
