...
interface AuthUser {
  ...
  // We'll use the isMessagesListPage defined below
  ...
  return (
    <>
      ...
      <Dialog open={showNewChatDialog} onOpenChange={setShowNewChatDialog}>
        <DialogContent className="bg-indigo-950 border-2 border-cyan-500/70">
          <DialogHeader>
            <DialogTitle className="text-cyan-300 retro-glow">INITIATE NEW TRANSMISSION</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={showCreateGroupChat} onOpenChange={setShowCreateGroupChat}>
        <DialogContent className="bg-indigo-950 border-2 border-cyan-500/70">
          <DialogHeader>
            <DialogTitle className="text-cyan-300 retro-glow">CREATE COSMIC GROUP</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-4">
            <div className="relative">
              <MessageSquare className="absolute left-3 top-1/2 transform -translate-y-1/2 text-cyan-300 h-4 w-4" />
              <RetroInput 
                placeholder="Group Name"
                className="pl-9"
                value={groupName}
            ...
            <div className="space-y-2">
              ...
              {searchResults.length > 0 && (
                <div className="mt-2 space-y-2 max-h-[30vh] overflow-y-auto border-2 border-cyan-600/30 p-2 rounded-lg bg-indigo-900/30">
                  {searchResults.map((user) => (
                    <div
                      key={user?.id}
                      className="flex items-center justify-between p-2 rounded bg-indigo-800/70 hover:bg-indigo-700/70 border-2 border-cyan-600/20"
                    >
                      <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        disabled={selectedUsers.includes(user?.id)}
                        className={`px-3 py-1 rounded-md border-2 ${selectedUsers.includes(user?.id) 
                          ? 'bg-indigo-900/50 border-cyan-700/30 text-cyan-700 cursor-not-allowed' 
                          : 'bg-cyan-600 border-cyan-500 text-white hover:bg-cyan-500 retro-btn'}`}
                      >
                        {selectedUsers.includes(user?.id) ? 'ADDED' : 'ADD'}
                      </button>
                    </div>
            ...
            {selectedUsers.length > 0 && (
              <div className="mt-4 border-2 border-cyan-600/30 p-3 rounded-lg bg-indigo-900/30">
                <h4 className="text-sm font-medium mb-2 text-cyan-300">SELECTED CREW MEMBERS:</h4>
                <div className="flex flex-wrap gap-2">
                  {selectedUsers.map((userId) => {
                    const user = searchResults.find(u => u.id === userId);
                    return (
                      <div
                        key={userId}
                        className="flex items-center gap-2 bg-indigo-800/70 rounded-md px-2 py-1 border-2 border-cyan-600/20"
                      >
            ...
            <button
              onClick={handleCreateGroup}
              disabled={!groupName || selectedUsers.length === 0}
              className={`w-full mt-4 py-3 rounded-md border-2 ${!groupName || selectedUsers.length === 0
                ? 'bg-indigo-900/50 border-cyan-700/30 text-cyan-700 cursor-not-allowed' 
                : 'bg-gradient-to-r from-cyan-600 to-indigo-600 border-cyan-500 text-white hover:from-cyan-500 hover:to-indigo-500 retro-btn'}`}
            >
              LAUNCH COSMIC GROUP
            </button>
          </div>
